...
public final class TypeResolver {
    ...
    private static Type reify(Type genericType, Map<TypeVariable<?>, Type> typeVariableMap) {
        if (genericType == null) {
            return null;
        }
        if (genericType instanceof Class) {
        ...
        if (genericType instanceof ParameterizedType) {
            ...
            Type[] reifiedTypeArguments = new Type[genericTypeArguments.length];
            boolean changed = false;
            for (int i = 0; i < genericTypeArguments.length; ++i) {
                reifiedTypeArguments[i] = TypeResolver.reify(genericTypeArguments[i], typeVariableMap);
                changed = changed || reifiedTypeArguments[i] != genericTypeArguments[i];
            }
            return changed ? new ReifiedParameterizedType(parameterizedType, reifiedTypeArguments) : parameterizedType;
        }
        if (genericType instanceof GenericArrayType) {
            Type reifiedComponentType;
            GenericArrayType genericArrayType = (GenericArrayType)genericType;
            Type genericComponentType = genericArrayType.getGenericComponentType();
            if (genericComponentType == (reifiedComponentType = TypeResolver.reify(genericArrayType.getGenericComponentType(), typeVariableMap))) {
        ...
        if (genericType instanceof TypeVariable) {
            ...
            if (mapping != null) {
                return TypeResolver.reify(mapping, typeVariableMap);
            }
            Type[] upperBounds = typeVariable.getBounds();
            return TypeResolver.reify(upperBounds[0], typeVariableMap);
        }
    ...
    private static class ReifiedParameterizedType
    implements ParameterizedType {
        ...
        public boolean equals(Object o) {
            ...
            return this.original.equals(that.original) && Arrays.equals(this.resolvedTypeArguments, that.resolvedTypeArguments);
        }

        public int hashCode() {
            int result = this.original.hashCode();
            result = 31 * result + Arrays.hashCode(this.resolvedTypeArguments);
            return result;
        }
    }
